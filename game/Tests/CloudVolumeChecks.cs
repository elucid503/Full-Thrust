...
public sealed partial class CloudVolumeChecks : Node {
    ...
    private async Task<float> Density(Vector3 point) {
    ...
    public override async void _Ready() {
        ...
        try {
            ...
            float baseline = await Density(point);
            Check(baseline > 0.1f, "interior cloud density remains substantial");
            Check(await Density(new Vector3(0, 1275000, 0)) < 0.01f, "cloud field is empty below its deck");
            Check(await Density(new Vector3(0, 1278100, 0)) < 0.01f, "cloud field is empty above its deck");
